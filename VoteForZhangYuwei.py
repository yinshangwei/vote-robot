...
def should_start_vote():
    ...
    vote_num_of_no1 = json.loads(response.text)['results'][0]['vote_num']
    vote_name_of_no1 = json.loads(response.text)['results'][0]['customer_name'].encode('utf-8')
    ...
    print '******************* Vote For 张雨薇********************'
    print '第一名({0})票数：{1}票'.format(vote_name_of_no1, '{:,}'.format(int(vote_num_of_no1)))
    print '第二名({0})票数：{1}票'.format(vote_name_of_no2, '{:,}'.format(int(vote_num_of_no2)))
    diff = int(vote_num_of_no1) - int(vote_num_of_no2)
    print '第一名({0})和第二名({1})相差：{2}票'.format(vote_name_of_no1, vote_name_of_no2, '{:,}'.format(diff))
    if diff < 30000:
        print '票数相差小于3万，开始投票...\n'
        return True
    else:
